...
const AIDashboard = () => {
  ...
  const analyzeData = async () => {
    ...
    try {
      ...
      const insightsResult =
        await aiInsightsEngine.generateInsights(dataContext);
  ...
  const generateRealPredictions = async (dataContext) => {
    try {
      ...
      if (dataContext.sheets.length >= 7) {
        ...
        try {
          const sheetsPrediction = RegressionPredictionService.predictLinear(
            sheetsData,
            7,
          );
          ...
          const monthlyPrediction = RegressionPredictionService.predictLinear(
            sheetsData,
            30,
          );
          ...
          predictions.nextWeek.sheets = Math.round(
            dataContext.sheets.length * 1.1,
          );
          predictions.nextMonth.sheets = Math.round(
            dataContext.sheets.length * 1.3,
          );
        }
      } else {
        predictions.nextWeek.sheets = Math.round(
      ...
      if (dataContext.files.length >= 7) {
        ...
        try {
          const filesPrediction =
            RegressionPredictionService.predictExponentialSmoothing(
              filesData,
              7,
            );
          ...
          predictions.nextWeek.files = Math.round(
            dataContext.files.length * 1.05,
          );
          predictions.nextMonth.files = Math.round(
            dataContext.files.length * 1.2,
          );
        }
      } else {
        predictions.nextWeek.files = Math.round(
      ...
      predictions.nextWeek.alerts = Math.max(
        0,
        Math.round(dataContext.alerts.length * 0.9),
      );
      predictions.nextMonth.alerts = Math.max(
        0,
        Math.round(dataContext.alerts.length * 0.8),
      );
  ...
  const generateSmartRecommendations = async (
    ...
    try {
      ...
      if (
        predictionsResult.nextMonth.sheets >
        predictionsResult.nextWeek.sheets * 3
      ) {
        recommendations.push({
          id: 2,
          ...
          description:
            "AI dự đoán tăng trưởng nhanh, cần chuẩn bị infrastructure scaling",
          priority: "high",
          effort: "high",
          impact: "high",
          confidence: 0.85,
      ...
      const anomalyInsights =
        insightsResult.insights?.filter(
          (insight) =>
            insight.type === "anomaly" || insight.category === "data_quality",
        ) || [];
      ...
      if (anomalyInsights.length > 0) {
        recommendations.push({
          ...
          description:
            "AI phát hiện anomalies, khuyến nghị strengthen monitoring system",
          priority: "high",
          effort: "low",
          impact: "high",
      ...
      const patternInsights =
        insightsResult.insights?.filter(
          (insight) =>
            insight.type === "pattern" || insight.category === "optimization",
        ) || [];
      ...
      if (patternInsights.length > 0) {
        recommendations.push({
          ...
          description:
            "AI phát hiện patterns có thể automate để giảm 40% manual work",
          priority: "medium",
          effort: "medium",
          impact: "high",
      ...
      if (recommendations.length === 0) {
        recommendations.push({
          ...
          description:
            "Maintain current monitoring approach và thu thập thêm data cho AI learning",
          priority: "low",
          effort: "low",
          impact: "medium",
          confidence: 0.7,
      ...
      return [
        {
          ...
          description:
            "Continue monitoring system performance và data patterns",
          priority: "medium",
          effort: "low",
          impact: "medium",
          confidence: 0.6,
    ...
    try {
      const insightsConfidence =
        insightsResult.insights?.length > 0
          ? insightsResult.insights.reduce(
      ...
      <div className="ai-performance-section">
        ...
        <div className="ai-metrics-grid">
          <div className="ai-metric-card">
            ...
            <div
              className={`metric-status ${aiPerformance.accuracy > 80 ? "good" : "warning"}`}
            >
          ...
          <div className="ai-metric-card">
            <div className="metric-icon">🧠</div>
            <div className="metric-value">
              {aiPerformance.insightsGenerated}
            </div>
