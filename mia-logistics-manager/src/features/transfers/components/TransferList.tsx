...
const getNumberFrom = (obj: Record<string, unknown>, key: string): number => {
  ...
  // Xử lý trường hợp string chứa toàn số 0 hoặc chuỗi rỗng
  if (typeof v === 'string') {
    const trimmed = v.trim();
    if (trimmed === '' || trimmed === '0' || /^0+$/.test(trimmed)) return 0;

    // Xử lý trường hợp string có format sai như "0.000.000.00" hoặc "42.000.000.000.000.000"
    if (/^0\.0+\.0+\.0+$/.test(trimmed) || /^\d+\.0+\.0+\.0+$/.test(trimmed))
      return 0;

    // Xử lý trường hợp string có dấu phẩy (thousands separator)
    let cleanValue = trimmed.replace(/,/g, '');

    // Xử lý trường hợp có nhiều dấu chấm (decimal separators)
    const dotCount = (cleanValue.match(/\./g) || []).length;
    if (dotCount > 1) {
      // Giữ lại dấu chấm cuối cùng làm decimal separator
    ...
    // Validation: đảm bảo kết quả hợp lý
    if (!Number.isFinite(n)) return 0;
    if (n > 1000000) return 0; // Giới hạn hợp lý cho volume
    if (n < 0) return 0; // Không cho phép số âm
...
const TransferList: React.FC = () => {
  ...
  useEffect(() => {
    console.log('🔍 Transfers state changed:', transfers);
    ...
    if (transfers.length > 0) {
      ...
      console.log('🔍 Component re-render count:', Date.now());
      console.log('🔍 Browser URL:', window.location.href);
      console.log('🔍 Local Storage:', localStorage.getItem('transfers'));
      console.log('🔍 Session Storage:', sessionStorage.getItem('transfers'));
      console.log('🔍 IndexedDB:', indexedDB.databases());
    }
  }, [transfers]);
    ...
    try {
      ...
      const mapped: Transfer[] = (data as Array<Record<string, unknown>>).map(
        (r) => ({
          ...
          transportStatus:
            getStringFrom(r, 'transportStatus') || 'Chờ báo kiện',
  ...
  // Load từ Google Sheets qua server nếu có cấu hình
  useEffect(() => {
    console.log('🚀 Component mounted, calling fetchTransfers...');
    fetchTransfers();
  }, []);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      console.log('🔄 Đã làm mới dữ liệu từ sheet');
    } catch (error) {
  ...
  const submitEdit = async () => {
    ...
    try {
      ...
      console.error('Cập nhật địa chỉ lỗi:', e);
    ...
    const districts = Array.from(
      new Set(
        results
          ...
          .filter(
            (s) =>
              s.toLowerCase().includes('quận') ||
              s.toLowerCase().includes('huyện')
          )
      )
    );
    ...
    const wards = Array.from(
      new Set(
        results
          ...
          .filter(
            (s) =>
              s.toLowerCase().includes('phường') ||
              s.toLowerCase().includes('xã')
          )
      )
    );
  ...
  const handleSubmitReport = async () => {
    ...
    try {
      ...
      if (locationRes.ok) {
        ...
        console.log(
          '🔍 Tìm kiếm địa điểm cho dest_id:',
          reportTransfer.dest_id
        );
        console.log('📋 Tổng số địa điểm:', locationData.length);
        ...
        if (foundLocation) {
          console.log('✅ Tìm thấy địa điểm:', foundLocation);
          locationInfo = {
          ...
          console.log(
            '❌ Không tìm thấy địa điểm cho dest_id:',
            reportTransfer.dest_id
          );
        }
      }
      ...
      if (r.ok) {
        ...
        const mapped: Transfer[] = (data as Array<Record<string, unknown>>).map(
          (rr) => ({
            ...
            transportStatus:
              getStringFrom(rr, 'transportStatus') || 'Chờ báo kiện',
    ...
    reader.onload = async (e) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          if (lines[i].trim()) {
            ...
            const transfer: Transfer = {
              ...
              hasVali: clean(values[2]) || '', // Có vali (cột 3)
        ...
        try {
          ...
          console.log('🔄 Reloading data from sheet after import...');
          await fetchTransfers();
  ...
  const fetchTransfers = async () => {
    console.log('🔍 fetchTransfers called...');
    setLoading(true);
    try {
      ...
      if (response.ok) {
        ...
        console.log('🔍 Setting transfers from fetchTransfers:', mapped);
        console.log('🔍 Transfers count from fetchTransfers:', mapped.length);
        setTransfers(mapped);
            ...
            columns={[
              ...
              },
              {
                ...
                render: (row: Transfer) => {
                  ...
                  return (
                    ...
                    >
                      {row.hasVali ? 'Có vali' : 'Không vali'}
                    </Typography>
                  );
                },
              },
              ...
              },
              {
                ...
                render: (row: Transfer) => {
                  ...
                  const isWarning =
                    status.includes('Xuất') || status.includes('Nhập');
                  const isSuccess =
                    status.includes('Đã') || status.includes('Hoàn');
              ...
              },
              {
                ...
                render: (row: Transfer) => {
                  ...
                  const isCompleted =
                    transportStatus.includes('Đã') ||
                    transportStatus.includes('Hoàn');
        ...
        <DialogTitle>
          ...
          >
            <Typography variant="h6">
              Thông số chi tiết phiếu chuyển kho
            </Typography>
            {gridIndex >= 0 && (
              <StatusChip
                status="primary"
                  ...
                  <Card sx={{ p: 2, height: '100%' }} elevation={1}>
                    ...
                    <Grid container spacing={1}>
                      ...
                      <Grid item xs={7}>
                        <StatusChip
                          status={(
                            gridTransfer.transportStatus || 'Chưa có'
                          ).toLowerCase()}
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12} sm={4}>
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="Phường/Xã" size="small" />
                )}
              />
            </Grid>
            <Grid item xs={12} sm={4}>
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="Quận/Huyện" size="small" />
                )}
              />
            </Grid>
            <Grid item xs={12} sm={4}>
            ...
            (() => {
              ...
              return (
                <Typography
                  ...
                  sx={{ mt: 0.5 }}
                >
                  {reportTransfer.orderCode} • {reportTransfer.dest} •{' '}
                  {totalPackages} Kiện (
      ...
      >
        <DialogTitle>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
            Vui lòng nhập thông tin địa điểm cho phiếu{' '}
