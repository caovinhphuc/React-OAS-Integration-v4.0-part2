...
const InboundSchedule: React.FC = () => {
  ...
  const {
    openDialog,
    openAddDialog,
    closeDialog: originalCloseDialog,
  } = useDialogs();
      ...
      if (editingItem) {
        ...
        const updated: InboundItem = {
          ...
          documentStatus: documentStatusItems.map((item) => {
            // Nếu có description mới, thêm vào history
            if (item.description && item.description.trim()) {
              return addDocumentStatusDescription(
                item,
                item.description,
                getCurrentUser()
              );
  ...
  const getDocumentDelayBreakdownByType = () => {
    ...
    return Object.entries(docTypeMap)
      .map(([docName, delays]) => ({
        docName,
        delays,
        totalCount: Object.values(delays).reduce(
          (sum, count) => sum + count,
          0
        ),
      }))
    ...
    return timelineOrder
      .map((timelineName) => ({
        ...
        totalCount: timelineTypeMap[timelineName]
          ? Object.values(timelineTypeMap[timelineName]).reduce(
              (sum, count) => sum + count,
              0
            )
          : 0,
  ...
  const getOnlineOfflineCount = () => {
    const onlineItems = filteredItems.filter(
      (item) => item.purpose === 'online'
    );
    const offlineItems = filteredItems.filter(
      (item) => item.purpose === 'offline'
    );
    ...
    filteredItems.forEach((item) => {
      ...
      if (item.packaging && item.packaging.length > 0) {
        ...
        categoryMap[category] =
          (categoryMap[category] || 0) + totalPackagingQty;
      } else {
        const qty = parseInt(item.quantity?.toString() || '0') || 0;
    ...
    <Box sx={{ p: 3 }}>
          ...
          >
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                <InventoryIcon
                  sx={{ fontSize: 42, color: 'primary.main', mr: 2 }}
                />
                <Box>
                  <Typography
                    variant="h4"
                ...
                >
                  ...
                  >
                    ...
                    {filteredItems.reduce(
                      (sum, item) =>
                        sum +
                        (parseInt(item.container?.toString() || '0') || 0),
                      0
                    )}{' '}
                    Containers
                  </Typography>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card sx={{ height: '100%' }}>
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                <TimelineIcon
                  sx={{ fontSize: 42, color: 'info.main', mr: 2 }}
                />
                <Box>
                  <Typography
                    variant="h4"
                  ...
                  >
                    {filteredItems.reduce(
                      (sum, item) => sum + (item.timeline?.length || 0),
                      0
                    )}
                  </Typography>
                  <Typography
              ...
              >
                {(() => {
                  const timelineDelayBreakdown =
                    getTimelineDelayBreakdownByType();
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card sx={{ height: '100%' }}>
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                <DocumentIcon
                  sx={{ fontSize: 42, color: 'success.main', mr: 2 }}
                />
                <Box>
                  <Typography
                    variant="h4"
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card sx={{ height: '100%' }}>
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                <ProductIcon
                  sx={{ fontSize: 42, color: 'warning.main', mr: 2 }}
                />
                <Box>
                  <Typography
                    variant="h4"
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {filteredItems.reduce(
                    (sum, item) =>
                      sum + (parseInt(item.container?.toString() || '0') || 0),
                    0
                  )}{' '}
                  Containers
                </Typography>
          ...
          <Grid container spacing={1}>
            {filteredItems
              .filter((item) => {
                ...
                const receiveDate =
                  item.actualArrival || item.estimatedArrival || item.date;
                ...
                return (
                  parsedDate.toDateString() === selectedDate.toDateString()
                );
              })
              .map((item) => (
      ...
      >
        <Box sx={{ p: 1 }}>
          <MenuItem
            onClick={() =>
              handleActionMenuAction('edit', selectedItemForAction!)
            }
          ...
          <MenuItem
            onClick={() =>
              handleActionMenuAction('delete', selectedItemForAction!)
            }
            sx={{ fontSize: { xs: '0.6rem', sm: '0.65rem' } }}
          >
